...
function initBlockly() {
    ...
    workspace.addChangeListener(function(event) {
        if (event.type == Blockly.Events.BLOCK_CHANGE || 
            ...
            // 아이템 사용 감지 블록 처리
            const itemUsedBlocks = blocks.filter(block => block.type === 'on_item_used');
            ...
            // 플레이어 동작 감지 블록 처리
            const playerTravelledBlocks = blocks.filter(block => block.type === 'on_player_travelled');
            playerTravelledBlocks.forEach(block => {
                const travelType = block.getFieldValue('TRAVEL_TYPE');
                const blockId = block.id;

                console.log('🚶 플레이어 동작 감지 블록 감지:', { travelType, blockId });
                socket.emit('updatePlayerTravelledCommand', { travelType, blockId });
                console.log('서버로 플레이어 동작 등록 전송 완료:', travelType);
            });
        }
    });
}
...
// 아이템 사용 등록 에러 처리
socket.on('itemUsedRegistrationError', function(data) {
    console.error('❌ 아이템 사용 등록 에러:', data.error);
    console.error('중복 아이템:', data.item);
    const itemDisplay = data.item === 'all' ? '모든 아이템' : data.item;
    showNotification(`❌ ${itemDisplay} 사용은 이미 등록되어 있습니다!`);

    // 중복 블록 자동 삭제
    setTimeout(() => {
        const blocks = workspace.getTopBlocks(true);
        const duplicateBlocks = blocks.filter(block =>
            block.type === 'on_item_used' &&
            block.id !== data.existingBlockId
        );
        ...
        duplicateBlocks.forEach(block => {
            const itemBlock = block.getInputTargetBlock('ITEM');
            let item = 'all';
            if (itemBlock && itemBlock.type === 'item_type') {
                item = itemBlock.getFieldValue('ITEM');
            }

            // 같은 아이템 타입의 중복 블록 찾아서 삭제
            if (item === data.item) {
                console.log('🗑️ 중복 블록 자동 삭제:', block.id);
                block.dispose(true);
            }
...
// 아이템 사용 이벤트 처리
socket.on('executeItemUsedCommands', async function(blockId) {
    if (isExecuting) {
        showNotification('이미 실행 중입니다.');
        return;
    }
    ...
    const blocks = workspace.getTopBlocks(true);
    const eventBlocks = blocks.filter(block => block.type === 'on_item_used');
...
// 플레이어 동작 감지 이벤트 처리
socket.on('executePlayerTravelledCommands', async function(data) {
    if (isExecuting) {
        showNotification('이미 실행 중입니다.');
        return;
    }
    ...
    const blocks = workspace.getTopBlocks(true);
    const eventBlocks = blocks.filter(block => block.type === 'on_player_travelled');

    const targetBlock = eventBlocks.find(block => block.id === data.blockId);
    if (targetBlock) {
        try {
            isExecuting = true;
            shouldStop = false;
            console.log('\n=== 플레이어 동작 실행 시작 ===');
            console.log('동작 타입:', data.travelType);
            console.log('플레이어:', data.playerName);
            console.log('블록 ID:', data.blockId);
            console.log('------------------------');
            showNotification(`플레이어 동작 감지: ${data.travelType}`);
            ...
            console.log('------------------------');
            console.log('=== 실행 완료 ===\n');
            showNotification('플레이어 동작 실행이 완료되었습니다.');
        } catch (e) {
            console.log('❌ 실행 중 오류 발생');
            console.error('오류 내용:', e);
            showNotification('실행 중 오류가 발생했습니다: ' + e.message);
        ...
        console.log('❌ 플레이어 동작 블록을 찾을 수 없음');
        showNotification('해당 플레이어 동작 블록을 찾을 수 없습니다.');
    }
});
